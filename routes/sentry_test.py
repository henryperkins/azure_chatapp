...
from utils.mcp_sentry import (
    get_issue_details, search_issues, enable_mcp_integrations,
    SentryMCPError, ServerConnectionError, ServerResponseError
)
...
async def test_sentry_message():
    ...
    sentry_sdk.capture_message(
        f"Test message from Sentry test endpoint with ID: {message_id}",
        level="info"
    )
...
async def test_sentry_performance(response: Response):
    ...
    with sentry_span(op="db.query", description="Simulate database query") as span:
        ...
        span.set_data("db.statement", "SELECT * FROM users WHERE active = true LIMIT 100")

        # Simulate a database operation
        time.sleep(0.1)  # 100ms simulated database query

    # Create a span for a simulated HTTP request
    with sentry_span(op="http.client", description="Simulate external API call") as span:
        ...
        with sentry_span(op="serialization", description="Process API response") as child_span:
...
async def test_sentry_mcp(issue_id: Optional[str] = None):
    ...
    try:
        ...
        if issue_id:
            with sentry_span(op="mcp.issue_details", description=f"Get issue details for {issue_id}"):
        ...
        # Otherwise, search for recent issues
            ...
            with sentry_span(op="mcp.search_issues", description="Search for recent issues"):
