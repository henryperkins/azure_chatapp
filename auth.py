...
async def login_user(
    ...
    logger.info("User '%s' logged in with token ID %s.", lower_username, token_id)
    ...
    set_secure_cookie(
        response, "access_token", token, max_age=ACCESS_TOKEN_EXPIRE_MINUTES * 60
    )
    set_secure_cookie(
        response, "refresh_token", refresh_token, max_age=60 * 60 * 24 * 7  # 7 days
    )
...
async def logout_user(
    ...
    blacklisted_token = TokenBlacklist(jti=token_id, expires=expires, user_id=user_id)
    ...
    logger.info(
        "User %s logged out successfully. Token %s invalidated and token version incremented.",
        user.username,
        token_id,
    )
