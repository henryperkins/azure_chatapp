      ...
      if (isProjectList) {
        ...
        return originalFetch.apply(this, args).then(async (response) => {
          ...
          try {
            ...
            // Hardened logic: check for arrays/objects in root and at .data, both
            let found = false;
            ...
            for (const key of arrayKeys) {
              if (Array.isArray(data[key])) {
                found = true;
                console.log(`[DEBUG-PROJECT] Found ${data[key].length} ${key}`);
                data[key].forEach((item, i) =>
                  console.log(`[DEBUG-PROJECT] ${key} ${i+1}:`, item)
                );
                break;
              }
              if (Array.isArray(data.data?.[key])) {
                found = true;
                console.log(`[DEBUG-PROJECT] Found ${data.data[key].length} ${key} (in .data)`);
                data.data[key].forEach((item, i) =>
                  console.log(`[DEBUG-PROJECT] ${key} (in .data) ${i+1}:`, item)
                );
                break;
              }
            }
            ...
            if (!found) {
              for (const key of objectKeys) {
                if (typeof data[key] === 'object' && data[key] && !Array.isArray(data[key])) {
                  found = true;
                  console.log(`[DEBUG-PROJECT] Found single ${key}:`, data[key]);
                  break;
                }
                if (typeof data.data?.[key] === 'object' && data.data[key] && !Array.isArray(data.data[key])) {
                  found = true;
                  console.log(`[DEBUG-PROJECT] Found single ${key} (in .data):`, data.data[key]);
                  break;
                }
              }
            }
