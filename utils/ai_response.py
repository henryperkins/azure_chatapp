...
async def retrieve_knowledge_context(
    ...
    try:
        ...
        for i, result in enumerate(search_results):
            ...
            filename = (
                file_info.get("filename", "Unknown source")
...
async def generate_ai_response(
    ...
    chosen_model = (
        "o1" if image_data else (model_id or "claude-3-7-sonnet-20250219")
    )  # Default to Claude 3.7 Sonnet

    try:
        # Get conversation to check for project context
        conversation = (
            await get_by_id(db, Conversation, conversation_id) if db else None
        )
        ...
        if knowledge_context:
            ...
            messages.insert(
                inject_index, {"role": "system", "content": knowledge_context}
            )
        ...
        if is_claude_model:
            ...
            max_response_tokens = (
                4000 if chosen_model == "claude-3-7-sonnet-20250219" else 1500
            )
            ...
            claude_response = await claude_chat(
                ...
                image_data=(
                    image_data if model_config.get("supports_vision", False) else None
                ),
            )
            ...
            if conversation and conversation.project_id and db:
                ...
                if chosen_model in CLAUDE_MODELS:
                    try:
                        ...
                        logger.warning(
                            f"Token counting failed, using estimation: {str(e)}"
                        )
