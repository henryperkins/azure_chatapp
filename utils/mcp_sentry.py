    ...
    def wrapper(*args, **kwargs):
        ...
        while retries < MAX_RETRIES:
            try:
                ...
                delay = RETRY_DELAY * (2 ** (retries - 1)) * (0.9 + 0.2 * (time.time() % 1))
...
def search_issues(query: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    # Create a span for this operation
    with sentry_sdk.start_span(op="mcp.request", description="Search Sentry Issues") as span:
...
def get_issue_events(issue_id: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    Raises:
        ...
        ServerResponseError: If the MCP server returns an error
    """
    logger.info(f"Fetching events for issue: {issue_id}, limit: {limit}")
    ...
    # Create a span for this operation
    with sentry_sdk.start_span(op="mcp.request", description=f"Get Events for Issue {issue_id}") as span:
...
def run_mcp_query(command_args: List[str], operation_name: str) -> Dict[str, Any]:
    ...
    # Create a span for this operation
    with sentry_sdk.start_span(op="mcp.request", description=f"Sentry MCP: {operation_name}") as span:
        try:
...
def get_project_stats(project_id: str, stat_type: str = "error", days: int = 14) -> Dict[str, Any]:
    """
...
def get_breadcrumbs(event_id: str) -> Dict[str, Any]:
    ...
    """
    return run_mcp_query(
        ["--get-breadcrumbs", event_id],
        "Get Event Breadcrumbs"
    )
...
def enable_mcp_integrations() -> bool:
    ...
    try:
        ...
        with configure_scope() as scope:
            ...
            scope.set_context("mcp_integration", {
                "enabled": True,
