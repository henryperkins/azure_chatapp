...
/* ---------------------------------------------------------------------------
   Dracula/Dark Mode Configuration Centralization
   ---------------------------------------------------------------------------
   All DaisyUI Dracula theme ("dracula" and "dracula-enhanced") variable
   definitions for this project are managed ONLY in this file.
   Do NOT attempt to duplicate or modify theme plugins elsewhere.
   To extend/adjust dark theme variables, add/change them here.
   ------------------------------------------------------------------------- */
...
/* Header user menu customization */
...
/* Enhanced Modals */
.modal-box {
  @apply max-h-[90vh] overflow-y-auto px-2;
...
@media (max-width: 640px) {
  ...
  /* lock background scroll while sidebar is open */
  html.sidebar-open,
  body.sidebar-open,
  body.chat-open{
    height:100dvh;          /* avoid iOS bounce */
    overflow:hidden;
  }

  /* chat message panes must fit viewport minus header/input */
  #projectChatMessages,
  #globalChatMessages{
    max-height:calc(100dvh - 220px);   /* tweak if header height changes */
    overscroll-behavior:contain;
    scrollbar-width:thin;
  }

  /* sticky input bar: safe-area + blur */
  ...
  /* hide desktop join-gap rule on phones (keep vertical stack) */
  @media (max-width:640px){
    #projectChatUI .flex.items-center.join{ gap:.5rem !important; }
  }

  /* enable smooth slide-in/out for sidebar */
  #mainSidebar{ transition:transform .3s ease; }
  /* =================================== */
}
...
/*
  Mobile override: On small screens, .text-xs is promoted to text-sm for readability.
  If you want true xs text on mobile, use .text-xs-force instead.
*/
@media (max-width: 640px) {
  .text-xs-mobile {
    font-size: 0.875rem !important; /* This is text-sm size. */
...
/* ===============================================================
   Mobile enhancements – screens ≤ 480 px
   ===============================================================*/
@media (max-width:480px){

  /* 1 – Dialogs: full-width “sheet” style */
  ...
  /* 2 – Touch targets: guarantee ≥ 44 × 44 px */
  .btn,
  .tab,
  .btn-square,
  .btn-circle,
  ...
  /* 4 – Typography: improve legibility on small screens */
  body,
  dialog.modal .modal-box,
  .project-card,
  .chat-message-container,
