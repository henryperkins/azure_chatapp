...
ARTIFACT_TYPES = {
    "code": ["python", "javascript", "typescript", "html", "css", "sql", "java", "cpp", "csharp", "go", "rust", "shell"],
...
async def create_artifact(
    ...
    if user_id is not None and project.user_id != user_id:
        raise HTTPException(status_code=403, detail="You don't have permission to add artifacts to this project")
...
async def list_artifacts(
    ...
    if user_id is not None:
        ...
        if project.user_id != user_id:
            raise HTTPException(status_code=403, detail="You don't have permission to access this project's artifacts")
    else:
...
async def get_artifact_stats(
    ...
    conv_query = select(
        ...
        Artifact.project_id == project_id,
        Artifact.conversation_id.isnot(None)
