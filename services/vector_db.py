...
# Fallback definitions so Pyright/Pylance recognizes these names
...
class VectorDB:
    ...
    async def generate_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        # Fallback (should never be reached if code is correct)
        return []
                ...
                if idx >= 0 and idx < len(self.id_map):  # Check for valid index
                    ...
                    if doc_id in self.metadata:
                        ...
                        results.append({
                            ...
                            "metadata": {
                                k: v
                                for k, v in self.metadata[doc_id].items()
                                if k != "text"
                            }
                        })
            return results
            ...
            for doc_id, vector in self.vectors.items():
                ...
                try:
                    ...
                    results.append({
                        ...
                        "metadata": {
                            k: v for k, v in self.metadata.get(doc_id, {}).items() if k != "text"
                        }
                    })
                except Exception as e:
